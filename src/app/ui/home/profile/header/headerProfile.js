...
export default async function HeaderProfile({ id }) {
	...
	const { full_name, user_name, avatar_url, description, followers, following } = data || {};
			...
			<>
				<div className="w-full flex justify-start items-start gap-4">
					...
					<div className="w-full h-full flex flex-col justify-between gap-3 text-sm">
						...
						<span className="text-gray-400 text-[1rem]">{description ? description : "No description"}</span>
					</div>
				</div>
			</>
		) : (
